...
:mtk
if not exist .\ramdisk\sbin\recovery (set ramdiskcfg=ROOTFS) else (set ramdiskcfg=RECOVERY)
"%tools%\mkimage.exe" zImage KERNEL >New_zImage
"%tools%\mkimage.exe" ramdisk-new.gz %ramdiskcfg% >New_ramdisk-new.gz
...
"%tools%\mkbootimg.exe" --kernel "New_zImage" --ramdisk "New_ramdisk-new.gz" --base %base% %cmdlinecfg% --pagesize %pagesize% %ramdisk_offsetcfg% %tags_offsetcfg% %second_offsetcfg% %dtcfg% -o "../New-%file%"
